# NetworkRequester
由于项目实现为siwft网络模块封装借鉴了YTKNetwork，但是阉割了很多功能和实现，由于后续需求更改，导致代码可读性、通用性下降，故编写此项目，解决一些问题。
...
类名|功能
-|-
BaseRequest|定义子类需覆盖的自定义参数方法及对应的默认参数
NetworkRequest|BaseRequest的子类。负责缓存的处理：请求前查询缓存；请求后写入缓存。所有的网络请求类需要继承于该类,每一个NetworkRequest类的子类代表一种专门的网络请求
NetworkUtils|工具类，处理一些加密、验证、序列化等工作
NetworkConfig|负责所有请求的全局配置，例如baseUrl和CDNUrl等等
NetworkAgent|该类为单例，负责管理所有的请求类（例如CustomRequest）。当CustomRequest发送请求以后，会把自己放在NetworkAgent持有的一个字典里，让其管理自己。实际上是NetworkAgent封装了Alamofire，由它负责Alamofire请求的发送和Alamofire的回调处理。如果我们需要第三方网络请求库，就可以在这里更换一下。而NetworkRequest更多的是只是负责缓存的处理。
